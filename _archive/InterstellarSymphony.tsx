  ...
  useEffect(() => {
    ...
    engine.runRenderLoop(() => {
        ...
        if (isAutoFlyoverRef.current) {
            ...
            // 3-phase loop — spaceship arc approach from off-side, throat dive, tube fly-through.
            const T_APPROACH = 5;
            const T_THROAT = 2.5;
            const T_TUBE = 14;
            const T = T_APPROACH + T_THROAT + T_TUBE;
            const cycleT = t % T;
            const sxC = Math.sin(t * 0.7) * 9 + Math.sin(t * 1.5) * 3;
            const syC = Math.cos(t * 0.6) * 9 + Math.cos(t * 1.3) * 3;
            const tA = t + 0.3;
            const sxCa = Math.sin(tA * 0.7) * 9 + Math.sin(tA * 1.5) * 3;
            const syCa = Math.cos(tA * 0.6) * 9 + Math.cos(tA * 1.3) * 3;
            const bankTube = Math.sin(t * 0.5) * 0.22 + Math.cos(t * 1.1) * 0.07;
            let z, sx, sy, lookX, lookY, lookZ, bank, fov;
            if (cycleT < T_APPROACH) {
                const u = cycleT / T_APPROACH;
                const ease = u * u * (3 - 2 * u);
                const e = ease, e1 = 1 - e;
                sx = e1*e1*9000 + 2*e1*e*5000 + e*e*0;
                sy = e1*e1*1500 + 2*e1*e*0 + e*e*0;
                z = e1*e1*7000 + 2*e1*e*1500 + e*e*400;
                lookX = 0;
                lookY = 0;
                lookZ = -2000;
                bank = -Math.sin(ease * Math.PI) * 0.45;
                fov = 0.85;
            } else if (cycleT < T_APPROACH + T_THROAT) {
                const u = (cycleT - T_APPROACH) / T_THROAT;
                const ease = u * u * (3 - 2 * u);
                z = 400 - 1500 * ease;
                sx = sxC * ease;
                sy = syC * ease;
                lookX = sxCa * ease;
                lookY = syCa * ease;
                lookZ = -2000 * (1 - ease) + (z - 1500) * ease;
                bank = bankTube * ease + 0.25 * Math.sin(u * Math.PI);
                fov = 0.85 + (1.1 - 0.85) * ease;
            } else {
                const u = (cycleT - T_APPROACH - T_THROAT) / T_TUBE;
                z = -1100 - 16900 * u;
                sx = sxC;
                sy = syC;
                lookX = sxCa;
