...
test('game-over CustomEvent drives the shell result overlay + save', async ({ page }) => {
  ...
  // Shell result overlay appears with the score, and the score was submitted.
  await expect(page.getByText(/game over/i)).toBeVisible({ timeout: 10_000 });
  await expect(page.getByText('4,200')).toBeVisible();
  await expect.poll(() => submits.length).toBeGreaterThan(0);
