...
// ============================================================================
// Board + Rules — chess.js-backed engine.
// Board stays as number[64] (wire-compatible with on-chain program); chess.js
// is constructed on demand from state for rules only. The on-chain Rust
// program remains authoritative — this is UX mirror only.
// ============================================================================
...
const idxToSq=(i:number):Square=>`${"abcdefgh"[i&7]}${(i>>3)+1}` as Square;
const sqToIdx=(s:string):number=>(s.charCodeAt(0)-97)+(parseInt(s[1])-1)*8;

function boardToFen(b:number[],whiteToMove:boolean,castle:number,ep:number):string{
  const rows:string[]=[];
  ...
  if(!cr)cr="-";
  const epStr=ep===255?"-":idxToSq(ep);
  return `${rows.join("/")} ${whiteToMove?"w":"b"} ${cr} ${epStr} 0 1`;
}

function loadChess(b:number[],whiteToMove:boolean,castle:number,ep:number):Chess|null{
...
function isAttacked(b:number[],sq:number,byW:boolean):boolean{
  // isAttacked is turn-independent; we set turn to !byW so FEN is generally legal.
...
function toAlg(f:number,t:number,p:number,b:number[],cap:boolean,ep:number,castle:number):string{
  const c=loadChess(b,isW(p),castle,ep);
  if(c){
    try{
...
export default function ChessGame(){
  ...
  const playSound=useCallback((type:"move"|"capture"|"check"|"castle"|"win"|"lose"|"invalid")=>{
    try{
      ...
      switch(type){
        ...
        case"invalid":osc.type="square";osc.frequency.value=120;gain.gain.setValueAtTime(0.2,t);gain.gain.exponentialRampToValueAtTime(0.001,t+0.15);osc.start(t);osc.stop(t+0.08);const o3=ctx.createOscillator();o3.type="square";o3.frequency.value=90;o3.connect(gain);o3.start(t+0.08);o3.stop(t+0.15);break;
      }
    }catch{}
  },[]);
  ...
  // Save score to SOAR + GPX1 when wallet connects after game
  const[soarSaved,setSoarSaved]=useState(false);
  const saveToSoar=useCallback(async()=>{
    if(!chainRef.current?.gamePda||!publicKey||soarSaved)return;
    try{
      const result={winner:won===true?"white" as const:won===false?"black" as const:"draw" as const,moves:mc};
      const ok=await chainRef.current.finish(publicKey.toBase58(),result);
      if(ok){
        addTx(`✓ Score + GPX1 game history saved to Solana`,"settle");
        setSoarSaved(true);
      }
    }catch(e:any){
      addTx(`⚠ Save failed: ${e.message?.slice(0,80)}`,"system");
    }
  },[publicKey,won,mc,soarSaved,addTx]);

  // Auto-save when wallet connects during gameover
  useEffect(()=>{
    if(phase==="gameover"&&publicKey&&!soarSaved&&chainRef.current?.gamePda){
      saveToSoar();
    }
  },[phase,publicKey,soarSaved,saveToSoar]);

  // Track streaks + stats on game end
  useEffect(()=>{
    if(phase!=="gameover")return;
    // Finish game on ER pool (without wallet — just recycle)
      ...
      if(chainRef.current?.gamePda){
        try{
          ...
          if(ok&&wallet){
            ...
            addTx("Game finished on ER","settle");
          }
          setMatchSettled(true);
        }catch(e:any){
          addTx(`⚠ Finish error: ${e.message?.slice(0,80)}`,"system");
          setMatchSettled(true);
        }
      }else{
  ...
  const click=useCallback((idx:number)=>{
    ...
    if(sel!==null&&valid.includes(idx)){
      ...
      // Send white move to ER, then AI move AFTER white confirms
      // This prevents desync between client and ER board state
      const erReady=chainRef.current?.isReady;
      const doErMoves=async(whiteSel:number,whiteIdx:number,whiteAlg:string)=>{
        if(!erReady)return;
        const wSig=await chainRef.current!.sendPlayerMove(whiteSel,whiteIdx,whiteAlg);
        if(wSig)addTx(`White: ${whiteAlg}`,"move",wSig);
        else addTx(`White: ${whiteAlg} (local)`,"move");
      };
      // Fire white move async — but track the promise for AI sequencing
      const whiteErPromise=doErMoves(sel,idx,alg);
      if(!erReady)addTx(`White: ${alg}`,"move");
      ...
      // AI — calculate move immediately (no delay for UX), but send to ER AFTER white confirms
      setTimeout(async()=>{
        ...
        // Wait for white move to confirm on ER, THEN send AI move
        if(erReady){
          await whiteErPromise;
          chainRef.current!.sendAiMove(pick.f,pick.t).then(sig=>{
            if(sig)addTx(`Black: ${r.alg}`,"move",sig);
            else addTx(`Black: ${r.alg} (local)`,"move");
          });
        }else{
      ...
      if(moves.length===0&&check){
        // Player tried to move a piece that can't resolve check
        playSound("invalid");
        triggerShake();
        setStatus("⚠ You're in check! Protect your king.");
      }
    }
  ...
  return(
    <div style={{minHeight:"100vh",background:lm?"#f5f5f5":"#050508",color:lm?"#222":"#e8e8f0",fontFamily:"'Space Grotesk', sans-serif",transition:"background 0.3s, color 0.3s"}}>
      ...
      {(phase==="ready"||phase==="playing")&&(
        <div style={{background:"linear-gradient(90deg, rgba(255,107,44,0.1), rgba(255,215,64,0.1))",borderBottom:`1px solid ${lm?"#ddd":"#252540"}`,padding:"6px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:11,flexWrap:"wrap",gap:4}}>
          <span style={{color:lm?"#b8860b":"#ffd740",fontWeight:600}}>🏆 Weekly Chess Tournament — Saturday 8pm UTC</span>
          {!isMobile&&<span style={{color:lm?"#666":"#888"}}>Entry: 50 $CHESS &bull; Prize: $500 pool &bull; <a href="/docs" style={{color:"#448aff",textDecoration:"none"}}>Learn more</a></span>}
        </div>
      )}
          ...
          <div style={{textAlign:"center",maxWidth:440}}>
            ...
            {!experience&&(
              <div style={{marginBottom:20}}>
                <p style={{color:"#555",fontSize:12,marginBottom:10}}>Choose your opponent</p>
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  {[
                    {id:"molty",label:"🦞 Molty",icon:"🦞",desc:"~600 ELO — Just learning, makes mistakes"},
                    {id:"coral",label:"🪸 Coral",icon:"🪸",desc:"~900 ELO — Plays safe, good for warming up"},
                    {id:"shadow",label:"🥷 Shadow",icon:"🥷",desc:"~1200 ELO — Club level, punishes blunders"},
                    {id:"neon",label:"⚡ Neon",icon:"⚡",desc:"~1600 ELO — Tournament level, fast and sharp"},
                    {id:"quantum",label:"🐡 Quantum",icon:"🐡",desc:"~2000 ELO — Near-master, deep calculation"},
                    {id:"zero",label:"🛡️ Zero",icon:"🛡️",desc:"~2400 ELO — Grandmaster level. Good luck."},
                  ].map(e=>(
                    <button key={e.id} onClick={()=>setExperience(e.id)} style={{
                      display:"flex",alignItems:"center",gap:12,padding:isMobile?"14px 16px":"12px 16px",
                      background:lm?"#f5f5f5":"#14141f",border:`1px solid ${lm?"#ddd":"#252540"}`,
                      borderRadius:8,cursor:"pointer",textAlign:"left",width:"100%",
                      transition:"border-color 0.15s",minHeight:isMobile?48:0,
                    }}
            ...
            {showPlayFair&&(
              <div style={{background:lm?"#fff":"#0c0c14",border:`1px solid ${lm?"#ddd":"#252540"}`,borderRadius:12,padding:24,textAlign:"center",marginTop:8}}>
                ...
                <button onClick={async()=>{
                  ...
                  const assigned=await chain.requestGame(experience||"medium");
                  if(assigned&&chain.isReady){
                    const ag=chain.agent;
                    addTx(`${ag?.emoji||"🤖"} Challenging ${ag?.name||"AI"} (${ag?.tierLabel||experience})`,"system");
      ...
      {(phase==="playing"||phase==="gameover")&&(
        <div style={{position:"relative",height:"calc(100vh - 56px)",overflow:"hidden"}}>
          ...
          <div style={{width:isMobile?(showTx?220:28):(showTx?260:36),transition:"width 0.2s",overflow:"hidden",flexShrink:0,display:"flex",flexDirection:"column",pointerEvents:"auto",background:viewMode==="3d"?(showTx?"rgba(10,0,20,0.85)":"rgba(10,0,20,0.6)"):(lm?"#fafafa":"transparent"),borderRight:viewMode==="3d"?"1px solid rgba(153,69,255,0.2)":`1px solid ${lm?"#ddd":"#252540"}`,backdropFilter:viewMode==="3d"?"blur(12px)":"none",position:isMobile?"absolute":"relative",zIndex:isMobile?10:1,height:isMobile?"100%":"auto",left:0}}>
            ...
            {phase==="gameover"&&(
              <div className="magic-chess-panel" style={{marginTop:12,textAlign:"center",borderRadius:12,padding:20,maxWidth:420,borderColor:won?"#14F195":won===false?"#ff1744":"#9945FF",boxShadow:`0 0 40px ${won?"rgba(20,241,149,0.15)":"rgba(255,23,68,0.1)"},0 0 60px rgba(153,69,255,0.15)`}}>
                  ...
                  <>
                    ...
                    {!publicKey?(
                      ...
                      <div style={{marginTop:12,padding:12,background:"rgba(20,241,149,0.08)",borderRadius:8,border:"1px solid rgba(20,241,149,0.2)"}}>
                        <div style={{fontSize:12,fontWeight:700,color:"#14F195",marginBottom:4}}>
                          {soarSaved?"Score + game history saved on Solana ✓":"Wallet connected ✓"}
                        </div>
                        <div style={{fontSize:9,color:"#888"}}>
                          {publicKey.toBase58().slice(0,4)}...{publicKey.toBase58().slice(-4)}
                          {soarSaved?" • SOAR + GPX1 permanent":""}
                        </div>
                        {soarSaved&&chainRef.current?.gamePda&&(
                          <a href={`/replay/${chainRef.current.gamePda.toBase58()}`}
                            style={{fontSize:10,color:"#9945FF",marginTop:4,display:"block"}}>
                            View replay →
                          </a>
                        )}
                        {!soarSaved&&(
                          <button onClick={saveToSoar} style={{marginTop:6,fontSize:10,padding:"4px 12px",background:"linear-gradient(135deg,#9945FF,#14F195)",border:"none",borderRadius:6,color:"#000",cursor:"pointer",fontWeight:600}}>
                            Save score + game history
                          </button>
                        )}
                      </div>
                    )}
                ...
                <div style={{display:"flex",gap:8,marginTop:12}}>
                  <button onClick={async()=>{
                    // Create a real challenge link
                    try{
                      const wallet=publicKey?.toBase58()||"anon";
                      const res=await fetch(`${process.env.NEXT_PUBLIC_RESOLVER_URL||"https://resolver.gamerplex.com"}/challenge/create`,{
                        method:"POST",headers:{"Content-Type":"application/json"},
                        body:JSON.stringify({creator:wallet,game:"chess",stake:0,referrer:wallet}),
                      });
                      const data=await res.json();
                      const link=data.ok?`gamerplex.com/challenge/${data.challenge.id}`:"gamerplex.com/play/chess";
                      const streakText=won&&streak>=3?` | ${streak}-win streak 🔥`:"";
                      const agentInfo=chainRef.current?.agent?` vs ${chainRef.current.agent.emoji} ${chainRef.current.agent.name}`:"";
                      const t=encodeURIComponent(`${won?"Checkmated":"Lost to"} an AI in Magic Chess on @gamerplex_com\n\n♟ ${mc} moves${agentInfo}${streakText}\n\nEvery move on @magaborgs MagicBlock ER (devnet beta)\n\nCan you beat me?\n${link}`);
                      window.open(`https://twitter.com/intent/tweet?text=${t}`,"_blank");
                    }catch{
                      const t=encodeURIComponent(`Playing Magic Chess on @gamerplex_com — every move on-chain via @magaborgs\n\ngamerplex.com/play/chess`);
                      window.open(`https://twitter.com/intent/tweet?text=${t}`,"_blank");
                    }
                  ...
                  <button className="magic-chess-btn" onClick={()=>{reset();setExperience(null);setShowPlayFair(false);setPhase("ready");setSoarSaved(false);}} style={{flex:1,padding:"10px",borderRadius:8,fontSize:13,cursor:"pointer"}}>✦ Play Again ✦</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{width:isMobile?(showBets?220:28):(showBets?260:36),transition:"width 0.2s",overflow:"hidden",flexShrink:0,display:"flex",flexDirection:"column",pointerEvents:"auto",background:viewMode==="3d"?(showBets?"rgba(10,0,20,0.85)":"rgba(10,0,20,0.6)"):(lm?"#fafafa":"transparent"),borderLeft:viewMode==="3d"?"1px solid rgba(153,69,255,0.2)":`1px solid ${lm?"#ddd":"#252540"}`,backdropFilter:viewMode==="3d"?"blur(12px)":"none",position:isMobile?"absolute":"relative",zIndex:isMobile?10:1,height:isMobile?"100%":"auto",right:0}}>
