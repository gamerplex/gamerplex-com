...
export class ChessOnChain {
  ...
  /** Agent info from challenge response */
  agent: { id: string; name: string; emoji: string; tier: number; style: string; tierLabel: string } | null = null;

  /**
   * Challenge an AI agent — creates an ephemeral game on MagicBlock ER.
   * Zero cost. Every move is a real Solana transaction.
   * @param agentId Agent ID or difficulty name ("easy", "hard", "shadow", etc.)
   */
  async requestGame(agentId: string = "shadow"): Promise<boolean> {
    try {
      const res = await fetch(`${RESOLVER}/game-pool/challenge`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ agentId, playerPubkey: "browser" }),
      });
      ...
      if (!data.ok || !data.game) {
        console.warn("Challenge failed:", data.error);
        return false;
      }

      const { gamePda, playerSecret, playerPublic, erRpc, aiPublicKey } = data.game;

      this.gamePda = new PublicKey(gamePda);
      this.sessionKey = Keypair.fromSecretKey(new Uint8Array(playerSecret));
      ...
      console.log(`[ER] Challenging ${data.agent?.emoji} ${data.agent?.name} (${data.agent?.tierLabel}) | Game: ${gamePda.slice(0, 12)}...`);
      return true;
    } catch (e: any) {
      console.error("[ER] Challenge failed:", e.message);
      return false;
    }
  }
