...
export default function Chess3DBoard({ board, selected, validMoves, lastMove, check, phase, onClick, autoRotate = false }: Chess3DProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (selectedRef.current !== null && state.pieceMeshes.has(selectedRef.current)) {
        ...
        controls.target.x += (0.5 - controls.target.x) * 0.03;
  ...
  useEffect(() => {
    ...
    if (checkRef.current) {
      ...
      if (whiteKingIdx >= 0) {
        const kingPos = idxToWorld(whiteKingIdx);
        const glowGeo = new THREE.RingGeometry(0.3, 0.6, 32);
        const glowMat = new THREE.MeshBasicMaterial({ color: 0xff1744, transparent: true, opacity: 0.7, side: THREE.DoubleSide });
        const glow = new THREE.Mesh(glowGeo, glowMat);
        glow.position.set(kingPos.x, 0.02, kingPos.z);
